...
then 
	echo "There must be a bind to '/var/run/docker.sock' (-v /var/run/docker.sock:/var/run/docker.sock)!!" >> /dev/stderr
	exit
fi
...
echo "Watching for changes in the '${ROUTING_NETWORK}' network .."
