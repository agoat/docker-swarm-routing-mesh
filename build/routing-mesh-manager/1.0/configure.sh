...
then 
	echo "A volume with the controller configuration (shared with the controller container) have to be declared ' (-v config:/etc/nginx/conf.d)!!" >> /dev/stderr
	exit
fi
...
do
	...
	echo ".. creating configuration for service/container '${service}' with domains: $domains"
	...
	if [ -n "$ssl" ]
	then
		echo -n "    preparing Let's Encrypt certificate ..."
